...
function ValidarTelefono1() {
	if (this.value.trim().length < 9 || this.value.trim().length > 10) {
		this.className="form-control error-input";
		this.nextSibling.innerHTML="¡Atención! El teléfono debe tener 9 o 10 dígitos";
		this.nextSibling.className="error";
	...
	if (inombre.value.trim() == 0) {
		...
		if (iapellidos.value.trim() == 0) {
			...
			if (idireccion.value.trim() == "") {
				...
				if (itelefono1.value.trim().length < 9 || itelefono1.value.trim().length > 10) {
					alert("El teléfono 1 debe contener 9 o 10 dígitos");
					info_evento.preventDefault();
				} else if (isNaN(itelefono1.value)) {
					alert("El teléfono 1 debe de ser numérico");
					info_evento.preventDefault();
				} else {
					if (itelefono2.value != "" && isNaN(itelefono2.value)) {
						alert("El teléfono 2 debe ser numérico");
						info_evento.preventDefault();
					} else {
